...
const TABS: { id: TabId; label: string; short: string }[] = [
  ...
  { id: "upgrades", label: "Upgrades", short: "Upgrades" },
  { id: "skills", label: "Skills", short: "Skills" },
  { id: "prestige", label: "Ascend", short: "Ascend" },
  { id: "quests", label: "Quests", short: "Quests" },
  { id: "achieve", label: "Badges", short: "Badges" },
...
export function AppShell() {
  ...
  return (
    <div className={`min-h-dvh flex flex-col ${s.settings.reducedMotion ? "reduce-motion" : ""}`}>
      <header className="sticky top-0 z-30 border-b border-white/10 bg-ink/80 backdrop-blur-md">
        <div className="mx-auto max-w-7xl px-3 py-2 flex flex-wrap items-center gap-2 justify-between">
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs sm:text-sm">
            ...
            <span className="hidden sm:inline">Slain {s.stats.monsters}</span>
          </div>
        </div>
        {activeBoosts.length > 0 && (
          <div className="mx-auto max-w-7xl px-3 pb-2 flex flex-wrap gap-2">
            {activeBoosts.map((b) => (
              <span key={b.id} className="text-[11px] rounded-full bg-void/30 border border-void/40 px-2 py-0.5 text-violet-200">
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-3 p-3 pb-28 lg:pb-3">
        <section className="panel relative min-h-[380px] sm:min-h-[420px] flex flex-col items-center justify-center p-4">
          <p className="text-xs uppercase tracking-[0.2em] text-slate-400">
            {s.bossActive ? "Boss" : `Wave ${s.killsInZone + 1}/10`} · {theme.name}
          </p>
          <h2 className="font-display text-2xl mt-1 text-center">{name}</h2>
          <button type="button" onClick={onArenaClick} className="relative rounded-full p-2 mt-2 touch-manipulation select-none" aria-label={`Attack ${name}`}>
            <div className={`${hit ? "monster-hit" : ""} w-40 h-40 sm:w-48 sm:h-48 rounded-full`} style={{ background: `radial-gradient(circle at 40% 35%, hsl(${theme.hue} 80% 62%), hsl(${theme.hue} 50% 18%))` }} />
            {g.floats.map((f) => (
              <span key={f.id} className={`float-num absolute font-semibold pointer-events-none ${f.crit ? "text-amber-300" : "text-white"}`} style={{ left: `${f.x}%`, top: `${f.y}%` }}>
                {f.crit ? "CRIT " : ""}{f.text}
              </span>
            ))}
          </button>
          <div className="w-full max-w-md mt-3">
            ...
            {s.bossActive && <p className="text-center text-ember text-sm mt-2 font-semibold">Timer {bossLeft}s</p>}
          </div>
        </section>

        <aside className="panel flex flex-col max-h-[65vh] lg:max-h-[calc(100dvh-7rem)] overflow-hidden">
          <nav className="hidden lg:flex flex-wrap gap-1 p-2 border-b border-white/10">
            {TABS.map((t) => (
              <button key={t.id} onClick={() => g.setTab(t.id)} className={`px-2 py-1 rounded-lg text-xs ${g.tab === t.id ? "bg-white/15 text-neon" : "text-slate-400"}`}>{t.label}</button>
            ))}
          </nav>
          <div className="flex-1 overflow-y-auto p-3 space-y-2">
                ...
                <article key={h.id} className="rounded-xl border border-white/10 p-3 bg-white/5">
                  <div className="flex justify-between gap-2 items-start">
                    <div className="min-w-0">
                      <h3 className="font-semibold truncate">{h.glyph} {h.name} {h.title}</h3>
                      <p className="text-xs text-slate-400 mt-0.5">{h.desc}</p>
                      <p className="text-xs text-neon mt-1">Lv {s.heroLevels[i] || 0} · {g.fmt(heroDps(i, s.heroLevels[i] || 0))} DPS</p>
                    </div>
                    <button disabled={locked || !gte(s.gold, cost)} onClick={() => g.buyHero(i)} className="shrink-0 rounded-lg px-3 py-2 text-xs bg-gold/20 text-gold disabled:opacity-30">{locked ? `Z${h.unlockZone}` : g.fmt(cost)}</button>
                  </div>
                </article>
              );
            })}
            ...
            {g.tab === "stats" && (
              <dl className="grid grid-cols-2 gap-2 text-sm">
                {[["Clicks", String(s.stats.clicks)], ["Damage", g.fmt(s.stats.totalDamage)], ["Gold", g.fmt(s.stats.totalGold)], ["Monsters", String(s.stats.monsters)], ["Bosses", String(s.stats.bosses)], ["Zone", String(s.stats.highestZone)], ["Ascensions", String(s.ascensions)], ["Play time", formatPlayTime(s.stats.playTime)], ["DPS", g.fmt(d.dps)], ["Click", g.fmt(d.clickDmg)]].map(([k, v]) => <div key={k} className="rounded-lg bg-white/5 p-2"><dt className="text-[10px] uppercase text-slate-500">{k}</dt><dd className="font-semibold">{v}</dd></div>)}
              </dl>
            )}
            {g.tab === "settings" && (
              <div className="space-y-3 text-sm">
                ...
                <button onClick={() => { void navigator.clipboard?.writeText(g.exportSave()); setImportMsg("Copied to clipboard"); }} className="w-full rounded-lg py-2 bg-white/10">Export save</button>
      ...
      <nav className="lg:hidden fixed bottom-0 inset-x-0 z-30 border-t border-white/10 bg-ink/95 pb-[env(safe-area-inset-bottom)]">
        <div className="flex overflow-x-auto gap-1 p-2">
          {TABS.map((t) => <button key={t.id} onClick={() => g.setTab(t.id)} className={`shrink-0 text-[11px] px-3 py-2.5 rounded-lg min-w-[4.5rem] ${g.tab === t.id ? "text-neon bg-white/15" : "text-slate-400"}`}>{t.short}</button>)}
        </div>
      </nav>

      <div className="fixed top-16 right-3 z-40 flex flex-col gap-2 max-w-xs pointer-events-none">
        {g.toasts.map((msg, i) => <div key={`${msg}-${i}`} className="rounded-lg bg-panel border border-gold/30 px-3 py-2 text-sm shadow-lg">{msg}</div>)}
      </div>

      {!s.introSeen && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4"><div className="panel max-w-md w-full p-6 space-y-3"><h2 className="font-display text-2xl gold-text">Welcome, Adventurer.</h2><p>Click the monster to attack.</p><p className="text-sm text-slate-400">Defeat enemies, earn Gold, hire Heroes, and conquer the Realm.</p><button onClick={() => g.clickMonster(50, 50)} className="w-full rounded-xl py-3 bg-gold text-ink font-semibold">Begin</button></div></div>}
      {g.offline && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4"><div className="panel max-w-md w-full p-6 space-y-2"><h2 className="font-display text-xl gold-text">Welcome Back!</h2><p className="text-sm">Away {formatDuration(g.offline.ms)}. ~{g.offline.kills} monsters.</p><p className="text-gold text-lg font-semibold">{g.fmt(g.offline.gold)} Gold</p><button onClick={g.collectOffline} className="w-full rounded-xl py-3 bg-gold text-ink font-semibold">Collect</button></div></div>}
